...
- 可能であれば、namespaceレベルで権限を割り当てます。
  特定のnamespace内でのみユーザーに権限を与えるため、ClusterRoleBindingではなくRoleBindingを使用します。
...
- `system:masters`グループにユーザーを追加しないでください。
  このグループのメンバーであるユーザーは、すべてのRBAC権限をバイパスし、常に制限のないスーパーユーザーアクセス権限を持ちます。この権限はRoleBindingまたはClusterRoleBindingを削除しても取り消すことができません。
